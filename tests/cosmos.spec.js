...
test('create Skeleton', async () => {
    ...
    const txSkeleton = txscosmos.createSkeleton(txContext);
...
test('delegate', async () => {
    ...
    const txDelegation = txscosmos.createDelegate(
        txContext,
        'val_addr',
        100,
        'some_memo',
...
test('redelegate', async () => {
    ...
    const txDelegation = txscosmos.createRedelegate(
        txContext,
        'val_addr_source',
        'val_addr_dest',
        100,
...
test('undelegate', async () => {
    ...
    const txDelegation = txscosmos.createUndelegate(
        txContext,
        'val_addr',
        100,
        'some_memo',
...
test('get bytes to sign', async () => {
    ...
    const jsonStr = txscosmos.getBytesToSign(txDelegation, txContext);
    const expectedJsonStr = '{"account_number":"0","chain_id":"test_chain","fee":{"amount":[{"amount":"5000","denom":"uatom"}],"gas":"200000"},"memo":"some_memo","msgs":[{"type":"cosmos-sdk/MsgDelegate","value":{"amount":{"amount":"100","denom":"uatom"},"validator_address":"val_addr"}}],"sequence":"0"}';
    console.log(jsonStr);
    expect(jsonStr).toBe(expectedJsonStr);
});
